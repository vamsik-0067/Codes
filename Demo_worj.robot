...
RSRP_SWEEP_TEST
    ...
    ${log} =       Login           oranlab    Ranzure@123
    Log    ${log}
    ${output} =    Start Command     cd C:\\platform-tools-latest-windows\\platform-tools && .\\adb shell "/data/local/tmp/iperf -s -i 1 -u -t 3600 -B ${ip} -p 6322 -P 5"> C:\\Logs\\hello_111.txt
    ...
    FOR    ${TARGET_RSRP}    IN    @{TARGET_RSRP_LIST}
	   ...
	   # Log    ${iperf_command}
	    ${iperf}=    Start Command    iperf -c ${ip} -u -i 1 -l 1350 -b 360m -t 3600 -p 6322 -B 192.168.205.2 -P 5 > iperf.txt
